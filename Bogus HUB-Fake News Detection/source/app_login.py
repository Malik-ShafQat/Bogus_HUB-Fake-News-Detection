...
def main_page():
    ...
    <h2 style="color:white;text-align:center;font-weight:bold"> Bogus HUB </h2>
    </div>
    <hr/>
    """
    st.markdown(html_temp, unsafe_allow_html=True)
    #st.image("./images/imageF.jpeg")
    st.image("./images/FNDBH.jpeg", caption=None, width=650, use_column_width="auto", clamp=False, channels="RGB", output_format="auto")
    st.sidebar.image(
        "./images/hand.gif",
        width=300,
    )
#    st.title("Login/Signup Bogus HUB ")
    menu = ["SignUp","Login","Home"]
    choice = st.sidebar.selectbox("Menu",menu)
    if choice == "Home":
            st.subheader("Verification Page")
            home_page()
    elif choice == "Login":
        st.title("Login Bogus HUB ")
        ...
        if st.sidebar.checkbox("Login"):
            ...
            if result:
                ...
                # menu1 = ["Verification Panel","About us"]
...
def scan_url(user_input):
    #client = vt.Client(os.environ.get("80b4b1c9c732d392bda11741415a4a625da7c023bf99db966a98cd3984c1215e"))
    client = vt.Client("80b4b1c9c732d392bda11741415a4a625da7c023bf99db966a98cd3984c1215e")
    try:    
        analysis = client.scan_url( str(user_input), wait_for_completion=True)
...
def home_page():
    ...
    user_input = str (st.text_input('Enter URL of an article or text'))
    ...
    if st.button('Check authenticity'):
        ...
        if article_text:
            with st.expander("🧙 Click here for more info about the article 🔮"):
        ...
        st.write("Rate Classfication Result")
        rt = ["Agree","Neutral","Disagree"]
        ch = st.selectbox("Rate",rt)
        if ch:
